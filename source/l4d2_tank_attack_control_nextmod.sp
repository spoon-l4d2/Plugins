...
public PutJumpRockOnCooldown(client)
{
	...
	JumpRockReady = false;
	CreateTimer(GetConVarFloat(g_hJumpRockCooldown), ResetJumpRockCooldown, client);
...
public Action:ResetJumpRockCooldown(Handle:timer, client)
{
	JumpRockReady = true;
	CPrintToChat(client, "<{red}JumpRock{default}> Jump Rock Is {olive}Ready!");
}

public Action Timer_Countdown(Handle timer, client)
{
